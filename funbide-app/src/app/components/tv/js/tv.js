...
const TURNOS_POR_ANUNCIO = new Set();
...
function desdeTurnos(turnos) {
  ...
  return {
    ...
    ticket_actual: turnosAtencion[0] || null,
    otros_llamados: turnosAtencion.slice(1),
    espera
  };
}
...
function actualizarUI(data) {
  ...
  if (cardsContainer) {
    if (turnosAtencion.length > 0) {
      const soloLlamando = turnosAtencion.filter(t => t.estado === 'Llamando');
      cardsContainer.innerHTML = soloLlamando.map(turno => {
      ...
      if (attentionCount) attentionCount.textContent = String(soloLlamando.length);
    } else {
...
function anunciarTurnoConCola(turno, puesto) {
  ...
  const llave = `${turno}:${puesto || 'modulo'}`;
  ...
  if (TURNOS_POR_ANUNCIO.has(llave)) return;

  const texto = generarTextoAnuncio(turno, puesto);
  ticketsLlamandoActual.add(turno);
  TURNOS_POR_ANUNCIO.add(llave);
  audioQueue.push({ turno, texto, puesto, id: turno });
  procesarColaAudio();
}

function generarTextoAnuncio(turno, puesto) {
  const numeroLimpio = String(turno).replace(/[^A-Z0-9-]/gi, ' ').replace(/\s+/g, ' ').trim();
  return `Turno ${numeroLimpio}. Favor pasar a atención.`;
}
...
function procesarColaAudio() {
  ...
  utterance.onend = () => {
    ...
    isPlaying = false;
    setTimeout(procesarColaAudio, RETRASO_ENTRE_SONIDOS + 1200);
  };
  utterance.onerror = () => {
    ticketsLlamandoActual.delete(item.id);
    isPlaying = false;
